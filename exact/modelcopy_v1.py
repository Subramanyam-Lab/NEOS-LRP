...
def create_data(file_loc):
    ...
    vehicle_cap = [vehicle_cap_value] * no_depot  
...
def subtourelim(model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        while unvisited:
            ...
            while stack:
                ...
                for neighbor in adj.get(node, []):
                    if neighbor in model._J and neighbor in unvisited:
        ...
        for S in components:
            if len(S) <= 1:
                continue
            D_S = sum(model._cust_dem[i - model._no_depot] for i in S)
            Q = model._vehicle_cap_value
            K = math.ceil(D_S / Q)
            max_edges = len(S) - K
            sum_xij = sum(vals[i,j] for i in S for j in S if i != j)
            if sum_xij - max_edges > 1e-6:
                expr = gp.quicksum(model._x[i,j] for i in S for j in S if i != j)
                model.cbLazy(expr <= len(S) - K)

def main():
    data_file = '/Users/waquarkaleem/NEOS-LRP-Codes-2/prodhon_dataset/coord20-5-1.dat'
    data = create_data(data_file)
    if data is None:
        return 

    [no_cust, no_depot, depot_cord, cust_cord, vehicle_cap_list, depot_cap, cust_dem, open_dep_cost, route_cost] = data

    vehicle_cap_value = vehicle_cap_list[0]
    ...
    model._vehicle_cap_value = vehicle_cap_value
    ...
    if model.SolCount > 0:
        ...
        print("\nRoutes:")
        routes = []  
        route_demands = []  # To store the total demand for each route
        for i in I:
            if y_vals[i] > 0.5:
                for j in J:
                    if x_vals[i,j] > 0.5:
